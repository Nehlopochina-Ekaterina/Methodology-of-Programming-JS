...
let coins = [
    {
        ...
        collected: false,
        draw: function () {
            if (!this.collected) { // Проверяем, собрана ли монетка
                noStroke();
                fill(184, 134, 11);
        ...
        checkCollision: function (player) {
            ...
            if (d < 30 && !this.collected) { // Если игрок касается монетки
                this.collected = true; // Помечаем монетку как собранную
                score.point += 2;
                return true; // Возвращаем true, если монетка собрана
            }
...
function drawCoins() {
    for (let coin of coins) {
        coin.draw(); // Вызываем метод draw для каждой монетки
    }
}

function checkCoinCollection(character) {
    for (let coin of coins) {
        coin.checkCollision(character); // Проверяем столкновение для каждой монетки
    }
}
...
function setup() {
    ...
    character = {
        ...
        canMove: function () {
            if (this.x >= innerWidth / 2.5) {
                if (this.state === "right" || this.state === "jumpRight") {
        ...
        checkCanyon: function () {
            ...
            let canyonLeft = canyons.x;
            let canyonRight = canyons.x + canyons.width;
    ...
    mountains = {
        ...
        drawMountains: function () {
            ...
            fill(120);
            triangle(this.x - innerWidth - 1012, this.y - this.h, this.x - innerWidth - 1890, this.y - this.h - 272, this.x - innerWidth - 1750, this.y - this.h);
        },
    ...
    /*coins = {
        ...
        draw: function () {
            ...
            fill(184, 134, 11);
            ellipse(this.x + 550, this.y, this.height, 30);
            fill(218, 165, 32);
            ellipse(this.x + 550, this.y, this.height - 5, 25);
...
function draw() {
    ...
    character.checkCollisionWithCoins();
    drawCoins();
    simple_enemy.draw();
